...
def main(input_folder=None, save_folder=None):
    """A multi-thread tool to crop sub imags."""
    if input_folder is None:
        input_folder = '/data/data/superresolution/DIV2K_train_HR/'
    if save_folder is None:
        save_folder = '/data/data/superresolution/DIV2K_train_HR_sub'
    n_thread = 28
    crop_sz = 480
    step = 240
    thres_sz = 48
    ...
    for path in img_list:
        pool.apply_async(worker,
            ...
            args=(path, save_folder, scale_percent, compression_level),
            callback=update)
    pool.close()
    pool.join()
...
def worker(path, save_folder, scale_percent, compression_level):
    ...
    cv2.imwrite(
        os.path.join(save_folder, img_name.replace('.png', '_s.png')),
        crop_img, [cv2.IMWRITE_PNG_COMPRESSION, compression_level])
